...
async def evaluate_pos(language_model_name, folder_path):
    ...
    original_mean_deviation_syllable_count = []
    original_correct_syllable_count = []
    original_correct_pos_lines = []
    original_pos_similarity = []

    print("Evaluating POS Constraint for " + language_model_name)

    constraint_folder_path = "Syllable_Constraint_|_POS_Constraint_|_/"

    async for index in atqdm(range(len(good_beamscore_multipliers_pos))):
        avg_perplexities_per_beam = []
        avg_syllable_differences_per_beam = []
        avg_mean_deviation_syllable_count_per_beam = []
        avg_correct_syllable_count_per_beam = []
        avg_correct_pos_lines_per_beam = []
        avg_pos_similarity_per_beam = []

        original_perplexities_per_beam = []
        original_syllable_differences_per_beam = []
        original_mean_deviation_syllable_count_per_beam = []
        original_correct_syllable_count_per_beam = []
        original_correct_pos_lines_per_beam = []
        original_pos_similarity_per_beam = []

        async for index_token in atqdm(range(len(good_token_multipliers))):
            temp_folder_path = folder_path + str(index*6 + index_token + 1) + "/" + language_model_name + "/" + constraint_folder_path +"/json/"
            if await aiofiles.os.path.isdir(temp_folder_path):
                perplexities = []
                syllable_differences = []
                mean_deviation_syllable_count = []
                ...
                for result in results:
                    ...
                    mean_deviation_syllable_count.append(result["mean_deviation_syllable_count"])
                    correct_syllale_count.append(result["nb_lines_correct_syllable_count"]/result["correct_nb_lines"])
                    correct_pos_lines.append(result["nb_lines_correct_pos_count"]/result["correct_nb_lines"])
                    pos_similarity.append(result["avg_pos_similarity"])

                avg_perplexities_per_beam.append(sum(perplexities)/len(perplexities))
                avg_syllable_differences_per_beam.append(sum(syllable_differences)/len(syllable_differences))
                avg_mean_deviation_syllable_count_per_beam.append(sum(mean_deviation_syllable_count)/len(mean_deviation_syllable_count))
                avg_correct_syllable_count_per_beam.append(sum(correct_syllale_count)/len(correct_syllale_count))
                avg_correct_pos_lines_per_beam.append(sum(correct_pos_lines)/len(correct_pos_lines))
                avg_pos_similarity_per_beam.append(sum(pos_similarity)/len(pos_similarity))

                original_perplexities_per_beam.append(perplexities)
                original_syllable_differences_per_beam.append(syllable_differences)
                original_mean_deviation_syllable_count_per_beam.append(mean_deviation_syllable_count)
                original_correct_syllable_count_per_beam.append(correct_syllale_count)
                original_correct_pos_lines_per_beam.append(correct_pos_lines)
                original_pos_similarity_per_beam.append(pos_similarity)

        avg_perplexities.append(avg_perplexities_per_beam)
        avg_syllable_differences.append(avg_syllable_differences_per_beam)
        avg_mean_deviation_syllable_count.append(avg_mean_deviation_syllable_count_per_beam)
        avg_correct_syllable_count.append(avg_correct_syllable_count_per_beam)
        avg_correct_pos_lines.append(avg_correct_pos_lines_per_beam)
        avg_pos_similarity.append(avg_pos_similarity_per_beam)

        original_perplexities.append(original_perplexities_per_beam)
        original_syllable_differences.append(original_syllable_differences_per_beam)
        original_mean_deviation_syllable_count.append(original_mean_deviation_syllable_count_per_beam)
        original_correct_syllable_count.append(original_correct_syllable_count_per_beam)
        original_correct_pos_lines.append(original_correct_pos_lines_per_beam)
        original_pos_similarity.append(original_pos_similarity_per_beam)
    ...
    async with aiofiles.open(model_folder + "/results.json", "w") as f:
        await f.write(json.dumps({
            ...
            "original_mean_deviation_syllable_count": original_mean_deviation_syllable_count,
            "original_correct_syllable_count": original_correct_syllable_count,
            "original_correct_pos_lines": original_correct_pos_lines,
            "original_pos_similarity": original_pos_similarity
        }, indent=4))
    
    plot_2d_heatmap(
        good_beamscore_multipliers_pos,
        good_token_multipliers,
        avg_perplexities,
        'Good Token Multiplier',
        'Good Beamscore Multiplier',
        'Perplexity',
        'Perplexity vs. Good Beamscore Multiplier and Good Token Multiplier',
        pos_folder+language_model_name.replace(" ", "_")+'/perplexity.png'
    )
    ...
    plot_2d_heatmap(
        ...
        'Good Beamscore Multiplier',
        'Syllable Differences',
        'Syllable Differences vs. Good Beamscore Multiplier and Good Token Multiplier',
        pos_folder+language_model_name.replace(" ", "_")+'/syllable_differences.png'
    )
    ...
    plot_2d_heatmap(
        ...
        'Good Beamscore Multiplier',
        'Mean Deviation Syllable Count',
        'Mean Deviation Syllable Count vs. Good Beamscore Multiplier and Good Token Multiplier',
        pos_folder+language_model_name.replace(" ", "_")+'/mean_deviation_syllable_count.png'
    )
    ...
    plot_2d_heatmap(
        ...
        'Good Beamscore Multiplier',
        'Correct Syllable Count',
        'Correct Syllable Count vs. Good Beamscore Multiplier and Good Token Multiplier',
        pos_folder+language_model_name.replace(" ", "_")+'/correct_syllable_count.png'
    )
    ...
    plot_2d_heatmap(
        ...
        'Good Beamscore Multiplier',
        'Correct POS Lines',
        'Correct POS Lines vs. Good Beamscore Multiplier and Good Token Multiplier',
        pos_folder+language_model_name.replace(" ", "_")+'/correct_pos_lines.png'
    )
    ...
    plot_2d_heatmap(
        ...
        'Good Beamscore Multiplier',
        'POS Similarity',
        'POS Similarity vs. Good Beamscore Multiplier and Good Token Multiplier',
        pos_folder+language_model_name.replace(" ", "_")+'/pos_similarity.png'
    )
